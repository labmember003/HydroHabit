...
import com.falcon.hydrohabit.composeapp.MainActivity
import com.falcon.hydrohabit.composeapp.R
...
class ComposeNotificationService(
    ...
    private fun getChannelId(soundIndex: Int): String {
        if (soundIndex == 6) {
            ...
            val customUri = prefs.getString("custom_sound_uri", null)
                ?: return "${CHANNEL_ID_PREFIX}6_default"
    ...
    fun showNotification(reminder: String) {
        ...
        val drinkPendingIntent = PendingIntent.getActivity(
            context, 1235, drinkIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )

        val snoozeIntent = Intent(context, ComposeSnoozeReceiver::class.java)
        val snoozePendingIntent = PendingIntent.getBroadcast(
            context, 1236, snoozeIntent,
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
